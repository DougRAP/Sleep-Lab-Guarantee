...
const turnbull = SEED_GUARANTEES.find((g) => g.customerLastName === "Turnbull")!;
...
describe("MemoryRepository — initial impression", () => {
  ...
  it("saves and reflects a first impression", async () => {
    ...
    const got = await repo.getInitialImpression(turnbull.id);
  ...
  it("updates (does not duplicate) when re-recording", async () => {
    const repo = new MemoryRepository();
    await repo.saveInitialImpression({ guaranteeId: turnbull.id, impression: "firmer" });
    await repo.saveInitialImpression({ guaranteeId: turnbull.id, impression: "just_right" });
    const got = await repo.getInitialImpression(turnbull.id);
...
describe("Tonight day-0 branch (impression prompt vs nightly check-in)", () => {
  ...
  it("shows the impression prompt for the fresh day-0 guarantee", async () => {
    const repo = new MemoryRepository();
    const journey = await repo.getJourney(turnbull.id);
    const impression = await repo.getInitialImpression(turnbull.id);
